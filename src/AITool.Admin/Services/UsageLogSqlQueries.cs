...
/// <summary>
/// 代理使用日志查询辅助类（基于 SqlSugar Ado 原生 SQL）。
/// <para>
/// 历史上用 Dapper + EF Core 连接绕过 EF Core 的 SQLite provider 对 DateTimeOffset 的 WHERE/ORDER BY 翻译限制。
/// 迁移到 SqlSugar 后改为通过 <see cref="ISqlSugarClient.Ado"/> 执行同样的参数化原生 SQL：
/// SqlSugar 原生支持 Guid/DateTimeOffset 映射，不再需要 Dapper 的 GuidTypeHandler。
/// </para>
/// <para>
/// 时间过滤、排序、分页、聚合、窗口函数全部在 SQLite 引擎端完成，避免全表加载到内存后客户端过滤。
/// </para>
/// </summary>
public static class UsageLogSqlQueries
{
        ...
        int pageSize)
    {
        ...
        var (where, parameters) = BuildWhereClause(rangeStart, rangeEnd, siteId, accessKeyId, source, status, modelKeyword);

        // 1. 查总数（COUNT 下推到 DB）
        var countSql = $"SELECT COUNT(*) FROM ProxyUsageLogs {where}";
        var totalCount = await db.Ado.SqlQuerySingleAsync<int>(countSql, parameters);
...
LIMIT @PageSize OFFSET @Offset";
        ...
        var items = await db.Ado.SqlQueryAsync<UsageLogRow>(listSql, parameters);
...
FROM ProxyUsageLogs {where}";
        ...
        var row = await db.Ado.SqlQuerySingleAsync<UsageLogSummaryRow>(sql, parameters);
        return row ?? new UsageLogSummaryRow();
    }

    /// <summary>
    /// 构建参数化的 WHERE 子句。julianday() 函数解析 TEXT 列为时间值（能解析带时区偏移的 ISO 8601）。
    /// </summary>
    private static (string Where, Dictionary<string, object?> Parameters) BuildWhereClause(
        DateTimeOffset rangeStart,
        DateTimeOffset rangeEnd,
        Guid? siteId,
        Guid? accessKeyId,
        ...
        string? modelKeyword)
    {
        ...
        // julianday() 能解析存储的带时区偏移的 ISO 8601 格式（datetime() 不支持时区）。
        ...
        if (rangeStart > DateTimeOffset.MinValue)
        {
            ...
            parameters["RangeStart"] = rangeStart.ToUniversalTime().ToString("O");
        }
        if (rangeEnd < DateTimeOffset.MaxValue)
        {
            conditions.Add("julianday(RequestedAt) < julianday(@RangeEnd)");
            parameters["RangeEnd"] = rangeEnd.ToUniversalTime().ToString("O");
        }
        ...
        if (accessKeyId.HasValue)
        {
            ...
            parameters["AccessKeyId"] = accessKeyId.Value;
        }
        ...
        if (!string.IsNullOrWhiteSpace(modelKeyword))
        {
            ...
            parameters["ModelPattern"] = $"%{EscapeLike(modelKeyword)}%";
        }
    ...
    /// 返回完整 UsageLogRow（ModelHealth 的聚合逻辑需要全部字段）。
    /// </summary>
    public static async Task<List<UsageLogRow>> QueryRecentForModelHealthAsync(
        ISqlSugarClient db, DateTimeOffset sinceCutoff)
    {
        var sql = @"
SELECT Id, RequestId, AccessKeyId, ProtocolType, RequestModel, AttemptedModel,
       TargetSiteId, Status, Source, RetryCount, AttemptIndex, IsFinalResult,
...
WHERE julianday(RequestedAt) >= julianday(@SinceCutoff)";
        ...
        return await db.Ado.SqlQueryAsync<UsageLogRow>(sql, new Dictionary<string, object?>
        {
        ...
        bool isAllRange)
    {
        ...
        if (isAllRange)
        {
            ...
            var minDateStr = await db.Ado.SqlQuerySingleAsync<string?>(minSql, parameters);
            ...
            parameters["EndTime"] = endTime.ToUniversalTime().ToString("O");
        }
        else
        {
            // 普通范围：上下界都加。
            ...
            conditions.Add("julianday(RequestedAt) < julianday(@EndTime)");
            parameters["StartTime"] = startTime.ToUniversalTime().ToString("O");
            parameters["EndTime"] = endTime.ToUniversalTime().ToString("O");
        }
...
FROM ProxyUsageLogs {where}";
        ...
        var items = await db.Ado.SqlQueryAsync<UsageLogRow>(sql, parameters);
    ...
    /// </summary>
    public static async Task<List<DetectionLatestLogRow>> QueryLatestFinalLogsAsync(ISqlSugarClient db)
    {
...
WHERE rn = 1";
        ...
        return await db.Ado.SqlQueryAsync<DetectionLatestLogRow>(sql);
    }
}

/// <summary>
/// 原生 SQL 映射的使用日志行。
/// bool 字段用 long 接收（SQLite 存 integer），RequestedAt 用 string 接收手动解析，避免格式问题。
/// </summary>
...
/// <summary>
/// 原生 SQL 映射的检测页最新日志行（轻量投影，仅展示所需字段）。
/// </summary>
