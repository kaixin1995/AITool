...
/// <summary>
/// Admin 独立宿主中的统计分析控制器，提供用量统计看板和趋势图表数据。
/// 与 Web 宿主版本不同，这里直接同步执行数据库查询，不依赖后台队列。
/// </summary>
[ApiController]
...
public sealed class AnalyticsApiController : ControllerBase
{
    ...
    /// 创建统计分析控制器。
    /// </summary>
    public AnalyticsApiController(AppDbContext dbContext)
    {
    ...
    /// <summary>
    /// 获取统计看板数据。
    /// Admin 独立宿主中直接同步查询数据库，省去后台队列和重试逻辑。
    /// </summary>
    [HttpGet("dashboard")]
    ...
    {
        var result = await BuildDashboardResponseAsync(_dbContext, query, cancellationToken);
        return Ok(result);
    }
