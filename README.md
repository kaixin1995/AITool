...
## AI 助手与公开数据源（1.0.1.26+）

- **AI 助手统一通道**（`AiAssistantService`）：协议诊断 AI 修复、模型价格 AI 补漏、
  请求头模板查最新版本共用一条 AI 调用链路；站点/模型在系统设置页一次选定、全系统生效；
  未配置时返回明确提示而非报错。
- **公开模型价格源**（models.dev / LiteLLM）：**零 AI 调用、秒级返回**的定价查询首选方式；
  双源并行拉取 + 单源 15s 超时 + 失败容忍；服务完全无状态零常驻（内存优先，用完即释放）。
- **客户端官方发布源**（GitHub Releases / npm）：查 Claude Code / Gemini CLI / Codex 等
  客户端最新版，先拉确定性数据再由 AI 归纳；30 分钟内存缓存；gzip 自动解压。
- **Codex 版本单源化**：上游客户端版本统一由 `CodexClientVersionResolver` 解析
  （远端模型目录优先），消除多处硬编码漂移。
- **模型价格一键清理**：价格页自动标记无匹配模型的无效条目，一键清理。
- **JSON diff 分段高亮**：字符串值内部的变化片段逐段高亮，变化点一眼可见。
- **暗色主题适配**：全局排查并修复硬编码深色文字/浅色背景导致的不可读问题。
