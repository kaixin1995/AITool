...
internal sealed class AdminAuthWebApplicationFactory : WebApplicationFactory<AITool.Admin.AdminProgramMarker>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            // 注册代理运行时基础设施，提供代理控制器所需的元数据缓存、熔断等基础服务。
            var spoolPath = Path.Combine(Path.GetTempPath(), $"aitool-admin-auth-spool-{Guid.NewGuid():N}");
            ...
            // 重新注册代理转发服务，覆盖基础设施注册的 HttpClient 版本。
            services.RemoveAll<IProxyForwardService>();
            services.AddSingleton<IProxyForwardService>(_fakeForwardService);

            // 注册 Core 独有的路由回退事件发布器，代理控制器构造函数需要此依赖。
            services.AddSingleton<AITool.Core.Services.CoreRouteFallbackEventPublisher>();

            // 重新注册 ProxyRequestMetadataCache，不使用配置快照路径，
            // 使缓存从数据库查询密钥数据，确保测试 seed 的数据可被读取。
            ...
            // 添加 Core 程序集到控制器发现，使 /v1/messages 等代理端点可用。
            services.AddControllers()
                .AddApplicationPart(typeof(AITool.Core.Controllers.Proxy.AnthropicProxyController).Assembly);
        });
    }
