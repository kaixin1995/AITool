...
import { getRouteFallbackSummary, listRouteFallbackEvents, type RouteFallbackEvent, type RouteFallbackSummary } from '@/api/routeFallback'
...
const summaryCards = computed(() => [
  { label: '回退总数', value: summary.value.totalCount },
...
async function loadSummary(): Promise<void> {
  summary.value = await getRouteFallbackSummary()
}
...
async function refreshAll(targetPage = page.value): Promise<void> {
  await Promise.all([loadList(targetPage), loadSummary()])
}
...
<template>
  <div class="page-container route-fallback-page">
    <PageHeader title="路由回退" subtitle="监控路由回退事件，诊断模型站点故障与降级情况" />
    ...
    <NCard content-style="padding: 0;" class="route-fallback-table-card">
      ...
      <div class="route-fallback-table-footer">
        ...
        <NPagination v-if="totalPages > 1" v-model:page="page" :page-count="totalPages" size="small" @update:page="loadList" />
