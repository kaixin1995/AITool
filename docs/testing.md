...
> 本文是 [README.md](../README.md) 的测试细节篇。当前 master 共 **2 个测试项目**：ApplicationTests 84 个执行用例 + IntegrationTests 241 个执行用例（Fact/Theory 展开 InlineData 后的实测数，`dotnet test` 于 2026-08-15 实跑确认；下文表格内的「用例」列为源码属性数，Theory 会展开为更多执行用例）；前端另有约 20 个 vitest 单测。
...
| 项目 | 定位 | 隔离手段 |
|------|------|----------|
| `tests/AITool.ApplicationTests`（xUnit + FluentAssertions） | 单元/服务测试（77 用例） | `TestDatabaseFactory.Create()`：每个测试 `%TEMP%/aitool-test-{GUID}.db` 临时 SQLite 文件，`SqlSugarSetup.InitializeDatabase` 建表，Dispose 删文件 |
| `tests/AITool.IntegrationTests`（xUnit + FluentAssertions + `WebApplicationFactory<Program>`） | 端到端集成（231 用例） | 每个测试工厂持有独立 `%TEMP%/aitool-<场景>-{GUID}.db`；`IntegrationTestDbHelper.ReplaceWithSqlSugar` 覆盖生产 SqlSugar 注册；`UseEnvironment("Testing")`；`IProxyForwardService` 换 Fake / Stub `HttpMessageHandler`，**不打真实外部 API** |
...
| `Proxy/RouteCircuitStateStoreTests.cs` | 6 | 熔断阈值触发/过期解除/失败连胜重计 |
| `Proxy/UsageLogServiceTests.cs` | 5 | 落库（总 token、fallback 元数据、失败分类、有界队列不丢条目） |
...
| `Proxy/ModelConcurrencyLimiterTests.cs` | 20 | SkipOnFull/WaitForSlot、站点/模型维度独立、动态调限、排队、DB 失败容错 |
