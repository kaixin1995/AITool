...
路由懒加载，`createWebHistory('./')`。守卫逻辑：公开页放行 → 拉取 auth status → 未设密码强制 `/login`（setup 模式）→ 未登录跳登录（returnUrl）→ 功能开关（`requiresCodex`/`requiresDeveloper` 不满足重定向 dashboard）。
...
- `[OAuth 管理]` 仅 `codexEnabled`；`[调试工具]` 仅 `developerEnabled`
...
| **CodexView**（1667 行，requiresCodex） | **两个页签**（URL `?tab=inspection` 同步）：「账号额度」— 账号卡片（状态 tag/planType/Token 过期 <3 天预警/额度窗口进度条 <20% error <50% warning/重置信用入口）、OAuth 登录弹窗、凭证上传/导出、编辑（改名/换 refresh_token/刷新 access_token）、拉取模型导入；「巡检」— 状态卡、上次结果（保留/禁用/启用/缓存命中/真实刷新计数 + 每账号明细表）、巡检日志，5s 轮询 + requestId 防旧响应 |
...
| **SystemSettingsView** | 分组卡：检测/代理（含并发策略与排队超时）/日志/开发者功能/Codex 巡检；数字项 Tooltip + 保存前整数校验（`systemSettingsState.ts`）；危险操作：按来源/时间清空日志 |
| **LoginView** | 单密码登录/首次设置双模式（isSetupMode），returnUrl 回跳 |

## 5. stores / composables / api 层

**stores/auth.ts**（唯一 Pinia store）：`accessToken`（localStorage）、`status`（hasPassword/features{codexEnabled, codexInspectionEnabled, developerEnabled}/version/buildTime）、actions：`fetchStatus`/`login`/`setup`/`logout`/`isAuthenticated`。
