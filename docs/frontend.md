...
| **OAuthView**（动态额度窗口，requiresOAuth） | **两个页签**（URL `?tab=inspection` 同步）：「账号额度」— 账号卡片（状态 tag/planType/Token 过期 <3 天预警/额度窗口进度条 <20% error <50% warning/重置信用入口）、OAuth 登录弹窗、凭证上传/导出、编辑（改名/换 refresh_token/刷新 access_token）、拉取模型导入；「额度巡检」— 状态卡、上次结果（保留/禁用/启用/缓存命中/真实刷新计数 + 每账号动态窗口明细表）、巡检日志，5s 轮询 + requestId 防旧响应 |
