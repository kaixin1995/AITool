...
> 本文是 [README.md](../README.md) 的 Codex 细节篇。Codex 托管把 chatgpt.com 的 Codex（OpenAI Responses 协议上游）以「OAuth 账号 → 隐藏站点」的方式接入网关，复用现有的路由/模型/对话测试/故障转移/并发控制全链路。
...
| 字段 | 含义 | 恢复方式 |
|------|------|----------|
| `DisabledByFeatureToggle` | 因总开关关闭被禁用（记录原 IsEnabled） | 重开 `CodexFeaturesEnabled` 时**仅**恢复此类 |
...
- `CodexFeaturesEnabled` 总开关：false 时禁用全部 Codex 托管站点 + 账号记 `DisabledByFeatureToggle`；控制器层 `CodexFeatureToggleAttribute`（ServiceFilter）整体 404；前端隐藏入口
- `CodexInspectionEnabled` 巡检开关：`CodexInspectionToggleAttribute` 对巡检 action 级 404
- `CodexInspectionIntervalSeconds`（下限 30s）、`CodexQuotaMaxCacheHours`（默认 6）、`CodexAutoDisableThresholdPercent`（默认 95）、`CodexInspectionCacheEnabled`
...
**登录流程（前端）**：`POST /api/admin/codex/start-oauth` 取授权 URL → 用户浏览器登录 → 粘贴回调 URL → `POST /complete-oauth`（L75）交换并建账号。也可上传 CPA 格式凭证 JSON（`import-credential`，多文件 multipart）——解析在 **`CodexCredentialParser.Parse`**（`Infrastructure/Codex/CodexCredentialParser.cs`）：type=codex 校验（缺失宽松推断）、JWT claims 权威回退顶层字段、过期时间优先 `expired` 再 JWT exp；`ParseMany` 单文件失败不中断。JWT 解析在 **`CodexJwtParser.Parse`**（`CodexJwtParser.cs`）：无验签解析 id_token → email/chatgpt_account_id/chatgpt_plan_type/订阅窗口（取自 `https://api.openai.com/auth` claim），按 token 内容缓存。`AccountId`（chatgpt_account_id）是去重首选依据。

**凭证导出**：`POST /accounts/export-credentials` 勾选多账号导出 `codex_credential_*.json`。
...
**`CodexInspectionService`**（Singleton + HostedService，`src/AITool.Web/Services/CodexInspectionService.cs`）：周期 `CodexInspectionIntervalSeconds` 执行一轮全账号额度巡检：
- **缓存复用 `QuotaCachePolicy.TryReuseQuota`**（静态，`Web/Services/QuotaCachePolicy.cs:27`）：`CodexInspectionCacheEnabled` 开启且账号未被使用（`SiteUsageTracker` 零 DB 查询，最近 7 天启动预热）且窗口未过期且未超 `CodexQuotaMaxCacheHours` 时沿用上次快照，减少上游请求
- 按账号级/全局 `CodexAutoDisableThresholdPercent` 阈值自动禁用额度耗尽账号（记录原因）
- `RunManualAsync(force)`（force=忽略缓存真实刷新）/`GetLastRun`/`GetLogs`/`GetStatus`
- 手动接口：`POST /inspection/run?force=`；结果页见 CodexView「巡检」页签（保留/禁用/启用/缓存命中/真实刷新计数 + 每账号 5h/周额度/动作/原因表）
...
## 8. 前端页面（CodexView，requiresCodex）
...
- **巡检**：状态卡（巡检中/空闲、上次完成、下次计划、手动巡检/真实巡检 force）、上次结果卡、巡检日志；未开启（404）显示空态；5s 静默轮询 + requestId 防旧响应覆盖（`codexInspectionState.ts`）
